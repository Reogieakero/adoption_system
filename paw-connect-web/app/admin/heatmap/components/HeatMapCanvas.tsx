...
function rescuePopupHtml(p: RescueHeatPoint): string {
  return `
    <div class="${styles.popup}">
      <span class="${styles.popupBadge}" style="background-color:${RESCUE_PIN_COLOR[p.priority]}">${escapeHtml(p.priority)} · Rescue</span>
...
export default function HeatMapCanvas({ rescuePoints, adoptionPoints, mode, view }: HeatMapCanvasProps) {
  ...
  // Heat layer — only active when view === 'heatmap'
  useEffect(() => {
    ...
    if (typeof (L as any).heatLayer !== 'function') {
      ...
      console.warn(
        '[HeatMapCanvas] L.heatLayer is not available — leaflet.heat did not attach correctly. ' +
    ...
    // Scale intensity to the *current* subset rather than a fixed 1.0 — if
  ...
  // Pins layer — only active when view === 'pins'
  useEffect(() => {
    const map = mapRef.current
    if (!map) return
